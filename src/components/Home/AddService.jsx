...
import ModalUbi from '../ui/ModalUbi';  // Asegúrate de importar tu componente ModalUbi correctamente
...
const SubmitButton = styled.button`
  ...
  height: 44px;
  font-size: 16px;
  background-color: ${(props) => (props.disabled ? '#cccccc' : 'var(--BtnColorPrincipal)')};;
  ...
  &:hover {
    background-color: ${(props) => (props.disabled ? '#cccccc' : 'var(--BtnColorPrincipalHover)')};
  }
`;
...
export default function RideService() {
    ...
    // Abre el modal para el primer paso
    const openModal = () => {
      setModalVisible(true);
    };
  
 // Cierra el modal, guarda la ubicación, y completa el paso 1
 const handleSaveLocation = (newLocation) => {
    setLocation(newLocation); // Actualiza la ubicación
    setStep1Completed(true); // Marca el paso 1 como completado
    setModalVisible(false);
  };

 // Completa el segundo paso con una alerta
 const completeStep2 = () => {
    alert('¡Paso 2 completado!');
    setStep2Completed(true); // Marca el paso 2 como completado
  };

  const completeStep3 = () => {
    alert('¡Paso 3 completado!');
    setStep3Completed(true); // Marca el paso 3 como completado
  };
  ...
  return (
    <>
      <Container>
        <div>
          <StepIndicator>Paso {stepsCompleted}/{totalSteps}</StepIndicator>
        ...
        <ButtonContainer>
          ...
          <SubmitButton disabled={!isAllStepsCompleted}>Buscar paseador</SubmitButton>
        </ButtonContainer>
      </Container>
      ...
      {isModalVisible && (
        <ModalOverlay onClick={() => setModalVisible(false)}>
          <ModalContent onClick={(e) => e.stopPropagation()}>
            {/* Modal para ingresar ubicación */}
            <ModalUbi onSave={handleSaveLocation} />
          </ModalContent>
        </ModalOverlay>
      )}
    </>
